...
exports.generateZegoToken = catchAsync(async (req, res, next) => {
  try {
    ...
    const payloadObject = {
      room_id: room_id, // Please modify to the user's roomID
...
exports.getCallLogs = catchAsync(async (req, res, next) => {
  ...
  // console.log("audiocall", audio_calls, "videocall", video_calls);

  for (let elm of audio_calls) {
    const missed = elm.verdict !== "Accepted";
    console.log("id", elm.from.id)
    console.log("elm", elm)
    console.log("_id", elm.from._id)
